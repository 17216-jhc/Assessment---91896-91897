...
def find_circum(r):
  ...
  circumference=round(p, 2) #rounds circumference to 2DP
  return circumference

def find_area(r):
  a=math.pi*r*r #calculates area using pi and the radius squared
  area = round(a, 2) #round area to 2DP
  return area
...
def circlecode():
  ...
  global tbox_circum #making tbox avaible to use in definitions outisde of trianglecode()
  ...
  tbox_circum=tk.Text(window,width=6,height=1,state="disabled",font=('Arial',24,"bold"))
   ...
   # Entry box for date, month and year
  global radius #making radius avaible to use in definitions outisde of trianglecode()
